...
export class IsAuthDirective extends SchemaDirectiveVisitor {
  public visitFieldDefinition(field) {
    ...
    field.resolve = async function (...args) {
      ...
      let err, authUser;
      [err, authUser] = await to(etudiant.findOne({ where: { num_etudiant: userInfo.num_etudiant } }));
      if (!authUser) {
        [err, authUser] = await to(professeur.findOne({ where: { num_prof: userInfo.num_prof } }));
